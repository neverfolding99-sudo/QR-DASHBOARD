...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btn) btn.textContent = 'Send til kundeskaerm: til';
  const dot = document.getElementById('liveFlag');
  if (dot) dot.classList.add('on');
  const flagText = document.getElementById('liveFlagText');
  if (flagText) flagText.textContent = 'Sender aflaest kode live til kundeskaerm';
});
...
function onDecoded(value) {
  ...
  const now = Date.now();
  // FIXED: throttle 300ms (was 1500ms) = 5x faster
  if (value === lastPushed && now - lastPushAt < 300) return;
  lastPushed = value;
  lastPushAt = now;
  pushToDisplay(value);
}
...
// FIXED: keep-alive ping every 4 min so Render never sleeps
setInterval(async () => {
